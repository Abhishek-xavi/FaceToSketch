...
from keras.layers import Lambda, Conv2D, add, Activation, UpSampling3D, multiply, BatchNormalization, UpSampling2D
...
PATH_P="/content/photo/*.jpg"
PATH_S="/content/sketch/*.jpg"


photo_data = import_images(PATH_P)
sketch_data = import_images(PATH_S)
train_dataset = tf.data.Dataset.from_tensor_slices((sketch_data, photo_data))
...
PATH_P="/content/photo_test/*.jpg"
PATH_S="/content/sketch_test/*.jpg"


photo_data = import_images_test(PATH_P)
sketch_data = import_images_test(PATH_S)
test_dataset = tf.data.Dataset.from_tensor_slices((sketch_data, photo_data))
...
def AttnGatingBlock(x, g, inter_shape):
    ...
    # Upscaling the number of filters in the lower level signal.
    phi_g = Conv2D(filters=inter_shape, kernel_size=1, strides=1, padding='same')(g)

    # Getting the x signal to the same shape as the gating signal
    theta_x = Conv2D(filters=inter_shape, kernel_size=3, strides=(2, 2), padding='same')(x)
    ...
    upsample_sigmoid_xg = Conv2D(filters=inter_shape, kernel_size=1, strides=1, padding='same')(upsample_sigmoid_xg)

    # Element-wise multiplication of attention coefficients back onto original x signal
    attn_coefficients = multiply([upsample_sigmoid_xg, x])

    # Final 1x1x1 convolution to consolidate attention signal to original x dimensions
    output = Conv2D(filters=inter_shape, kernel_size=1, strides=1, padding='same')(attn_coefficients)
...
def Generator():
  up_stack_filter = [512, 512, 512, 512, 256, 128, 64]
  ...
  for up, skip, inter_shape in zip(up_stack, skips, up_stack_filter):
    x_up = up(x)
    att_gate_output = AttnGatingBlock(skip, x, inter_shape)
    x = tf.keras.layers.Concatenate()([x_up, att_gate_output])
